...
class PadZero(DisplayedTransform):
    ...
    def __init__(self, seq_len=32, **kwargs):
...
class CharBlock(TransformBlock):
    ...
    def __init__(self, vocab=None, seq_len=32, **kwargs):
...
class SwapAxes(DisplayedTransform):
    def encodes(self, batch):
        batch[1] = torch.swapaxes(batch[1], 1, 2)
        return batch

    def decodes(self, batch):
        return self.encodes(batch)
